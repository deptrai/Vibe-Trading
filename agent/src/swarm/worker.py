...
def build_worker_prompt(
    ...
    prompt_parts.append(
        ...
        "**Output contract (the runtime WILL reject your run if you skip this):**\n"
        "- BEFORE you write `report.md`, you MUST have called `bash` at least once in this run "
          "to actually execute one of your Python scripts (e.g. `bash python btc_analysis.py`). "
          "`write_file`, `read_file`, and `load_skill` do NOT count — only an executed `bash` "
          "call (or `factor_analysis` if available) qualifies as data evidence.\n"
        "- The validator counts non-generic tool calls. If you only `load_skill` + `write_file` and "
          "then submit `report.md`, the run is marked `incomplete` and you fail the task. There is "
          "no way around this — you must run at least one script via `bash`.\n\n"
        "**Finalize (MUST use write_file):**\n"
        "- After at least one successful `bash` run, you MUST call `write_file` with path "
          "`report.md` to save your final report as a markdown file.\n"
        ...
        "- The report must include specific numbers, dates, and actionable conclusions — "
          "use numbers produced by your executed scripts, not numbers from training data.\n"
